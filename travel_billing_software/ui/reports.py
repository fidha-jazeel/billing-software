    ...
    def export_to_excel(self):
        ...
        try:
            ...
            filename, _ = QFileDialog.getSaveFileName(
                ...
                f"invoices_report_{QDate.currentDate().toString('yyyy-MM-dd')}.xlsx",
                "Excel Files (*.xlsx)"
            )
            ...
            # Headers
            headers = ["Invoice #", "Date", "Customer", "Contact", "Subtotal", "Discount", "Tax", "Total", "Received", "Balance", "Status"]
            ws.append(headers)
            ...
            # Load all invoices from JSON files
            invoice_dir = self.invoice_config.get('save_directory', 'invoices')
            
            if os.path.exists(invoice_dir):
                for filename_json in sorted(os.listdir(invoice_dir)):
                    if filename_json.endswith('.json'):
                        filepath = os.path.join(invoice_dir, filename_json)
                        try:
                            ...
                            contact = invoice_data.get('contact_number', '')
                            subtotal = invoice_data.get('subtotal', '₹0.00').replace('₹', '').replace(',', '').strip()
                            discount = invoice_data.get('discount', '0.00').replace('₹', '').replace(',', '').strip()
                            tax = invoice_data.get('tax', '₹0.00').replace('₹', '').replace(',', '').strip()
                            total = invoice_data.get('total', '₹0.00').replace('₹', '').replace(',', '').strip()
                            received = invoice_data.get('received', '₹0.00').replace('₹', '').replace(',', '').strip()
                            balance_text = invoice_data.get('balance', '₹0.00').replace('₹', '').replace(',', '').replace('(Paid)', '').replace('(Overpaid)', '').strip()
                            
                            # Determine status
                            try:
                                balance_val = float(balance_text) if balance_text else 0
                                if balance_val == 0:
                                    status = "Paid"
                                elif balance_val < 0:
                                    status = "Overpaid"
                            ...
                            row = [
                                ...
                                customer,
                                contact,
                                float(subtotal) if subtotal else 0,
                                float(discount) if discount else 0,
                                float(tax) if tax else 0,
                                float(total) if total else 0,
                                float(received) if received else 0,
                                float(balance_text) if balance_text else 0,
                                status
                            ]
                            ws.append(row)
                        except Exception as e:
                            print(f"Error reading invoice {filename_json}: {e}")
                            continue
            ...
            # Calculate column totals (E to J)
            for col_idx, col_letter in enumerate(['E', 'F', 'G', 'H', 'I', 'J'], start=5):
                ws[f"{col_letter}{last_row}"] = f"=SUM({col_letter}2:{col_letter}{last_row-2})"
                ws[f"{col_letter}{last_row}"].font = Font(bold=True)
                ws[f"{col_letter}{last_row}"].border = border
            ...
            QMessageBox.information(
                self,
                "Export Successful",
                f"Invoices exported successfully!\n\n{filename}\n\nTotal: {ws.max_row - 1} invoices"
            )
